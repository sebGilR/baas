    ...
    class RefreshTokenService < ApplicationService
      ...
      def rotate_tokens(old_token, user, account)
        ActiveRecord::Base.transaction do
          old_token.revoke!
          new_tokens = create_new_tokens(user, account, old_token.device_info)
          new_tokens
        end
      end

      def create_new_tokens(user, account, device_info)
        ...
        { access_token: access_result.data.access_token, refresh_token: new_refresh_token }
      end
    end
  end
end
