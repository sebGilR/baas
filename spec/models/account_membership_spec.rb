...
RSpec.describe(AccountMembership, type: :model) do
  describe "validations" do
    subject(:membership) { build(:account_membership) }

    it {
      expect(membership).to(validate_uniqueness_of(:user_id)
        .scoped_to(:account_id)
        .with_message("already a member of this account"))
    }
  end
  ...
  describe "enums" do
    ...
    it {
      expect(membership).to(define_enum_for(:role)
        .with_values(owner: 0, admin: 1, editor: 2, author: 3, viewer: 4)
        .with_prefix)
    }
  ...
  describe "scopes" do
    ...
    it "returns only active memberships" do
      ...
      expect(described_class.active).not_to(include(invited_membership))
    end

    it "returns memberships for a specific account" do
      expect(described_class.for_account(account)).to(include(membership_for_account))
      expect(described_class.for_account(account)).not_to(include(other_membership))
    end
  end
